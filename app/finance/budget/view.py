...
def set_budget():
    ...
    try:
        Budget.upsert(int(category_id), int(year), int(month), float(amount))
...
def update_budget(id):
    ...
    try:
        if not Budget.update(id, float(data['amount'])):
