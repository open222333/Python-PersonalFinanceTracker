...
| **定期交易** | 設定定期自動收支項目（每月薪水、房租等），一鍵轉為正式記錄 |
| **預算管理** | 設定各分類每月預算，即時顯示使用率與超支警示 |
| **報表分析** | 年度收支趨勢長條圖、月份支出分類圓餅圖 |
| **資料匯出** | 一鍵匯出 CSV（UTF-8 BOM）或 Excel（.xlsx）|
| **手機快速記帳** | 行動裝置最佳化大按鈕介面，支援收支與股票快速紀錄（`/finance/quick/`）|
| **使用者帳號** | JWT 認證、自助註冊（可控開關）、記住我 30 天、帳號資料完全隔離 |
...
│   ├── auth/view.py                # /auth/* 登入、註冊、JWT
│   ├── user/view.py                # 使用者 CRUD
│   ├── admin/view.py               # 系統後台 UI
│   ├── docs/view.py                # GET /docs/ → 使用說明頁面
│   ├── finance/
...
│   │   ├── recurring/view.py       # /finance/recurring/* 定期交易
│   │   ├── quick/view.py           # GET /finance/quick/ → 手機快速記帳頁面
│   │   └── dashboard/view.py       # GET /finance/ → 前端 SPA 頁面
│   └── templates/
│       ├── admin/index.html        # 系統管理後台
│       ├── docs/index.html         # 使用說明文件頁面
│       └── finance/
│           ├── dashboard.html      # 理財追蹤 SPA（主要介面）
│           └── quick.html          # 手機快速記帳（行動版）
│
├── src/
│   ├── mysql.py                    # MySQL 連線池
│   ├── models/
...
│       └── host/
│           ├── http.conf                               # 主機 nginx — 純 HTTP
│           ├── cloudflare.conf                         # 主機 nginx — Cloudflare Origin CA SSL
│           └── https-letsencrypt.conf                  # 主機 nginx — Let's Encrypt SSL
│
├── docker/
...
| 📱 手機快速記帳 | http://127.0.0.1:5000/finance/quick/ |
| 🛠 系統後台管理 | http://127.0.0.1:5000/admin/ |
| 📖 使用說明文件 | http://127.0.0.1:5000/docs/ |
...
| 📱 手機快速記帳 | http://localhost/finance/quick/ |
| 🛠 系統後台管理 | http://localhost/admin/ |
| 📖 使用說明文件 | http://localhost/docs/ |
...
> 適用情境：VPS 不使用 Docker，直接以 **gunicorn + nginx** 在主機上執行 Flask 應用。

```
使用者 → nginx:80/443 → 127.0.0.1:5000（gunicorn）→ MySQL / MongoDB / Redis（主機服務）
```
...
# MongoDB 需加入官方 APT 倉庫後安裝（依 Ubuntu 版本而異）
...
> **注意：請勿將專案放在 `/root/` 下。**  
> nginx 以 `www-data` 執行，`/root/` 權限為 `700`，nginx 無法讀取，會出現 `(13: Permission denied)` 錯誤。  
> 若已部署在 `/root/`，請先搬移：
> ```bash
> sudo mv /root/Python-PersonalFinanceTracker /opt/finance
> sudo chown -R www-data:www-data /opt/finance
> ```
...
`conf/flask.json` 的 `SECRET_KEY` 若為空字串，首次啟動時會自動產生並寫回。

### Step 4：建立 MySQL 資料庫與資料表

```bash
# 建立資料庫與使用者
sudo mysql -e "CREATE DATABASE IF NOT EXISTS flask_app CHARACTER SET utf8mb4;"
sudo mysql -e "CREATE USER IF NOT EXISTS 'flask_user'@'localhost' IDENTIFIED BY 'your_password';"
sudo mysql -e "GRANT ALL PRIVILEGES ON flask_app.* TO 'flask_user'@'localhost'; FLUSH PRIVILEGES;"

# 建立 4 張資料表
sudo mysql flask_app < /opt/finance/db/finance_schema.sql
```

### Step 5：建立 systemd 服務（gunicorn）
...
```ini
[Unit]
Description=Personal Finance Tracker (gunicorn)
...
Environment="PATH=/opt/finance/venv/bin"
ExecStart=/opt/finance/venv/bin/gunicorn \
    --workers 2 \
    --bind 127.0.0.1:5000 \
    --timeout 120 \
    run:app
...
選擇一種模式，複製對應的 host 設定檔：

```bash
# ── 模式一：HTTP（IP 直連或 Cloudflare 代理）───────────────
sudo cp /opt/finance/conf/nginx/host/http.conf /etc/nginx/sites-available/flask-app
sudo nano /etc/nginx/sites-available/flask-app
# 將 YOUR_DOMAIN 改為實際域名，或改為 _ 接受所有請求

# ── 模式二：Cloudflare Origin CA SSL ──────────────────────
# 前置：建立 Cloudflare 憑證並放置於 /etc/ssl/cloudflare/（參見檔案頂部說明）
sudo cp /opt/finance/conf/nginx/host/cloudflare.conf /etc/nginx/sites-available/flask-app
sudo nano /etc/nginx/sites-available/flask-app
# 將 YOUR_DOMAIN 替換為實際域名（共 2 處）

# ── 模式三：Let's Encrypt SSL ──────────────────────────────
sudo apt install -y certbot python3-certbot-nginx
# 先以 http.conf 啟動 nginx 後，再申請憑證：
sudo certbot certonly --nginx -d your.domain.com
# 憑證申請成功後換用 letsencrypt 設定：
sudo cp /opt/finance/conf/nginx/host/https-letsencrypt.conf /etc/nginx/sites-available/flask-app
sudo nano /etc/nginx/sites-available/flask-app
# 將所有 YOUR_DOMAIN 替換為實際域名（共 4 處）
```
...
```bash
curl http://127.0.0.1:5000/finance/   # 確認 gunicorn 直接回應
curl http://localhost/finance/         # 確認 nginx 反向代理正常
```

首次啟動 gunicorn 時，`run.py` 會自動建立 `admin / admin` 帳號並插入 16 個預設財務分類，**請立即登入修改密碼**。

| 服務 | 網址 |
|---|---|
| 📊 個人理財追蹤系統 | http://your.domain/finance/ |
| 📱 手機快速記帳 | http://your.domain/finance/quick/ |
| 🛠 系統後台管理 | http://your.domain/admin/ |
| 📖 使用說明文件 | http://your.domain/docs/ |
| 📄 Swagger API 文件 | http://your.domain/apidocs |
...
```bash
# gunicorn 服務
sudo systemctl status finance          # 服務狀態
sudo systemctl restart finance         # 重啟（程式碼修改後）
sudo journalctl -u finance -f          # 即時日誌
...
# MySQL
sudo mysql flask_app                   # 進入 MySQL CLI
sudo mysqldump flask_app > backup.sql  # 備份資料

# 更新程式碼
cd /opt/finance && git pull
...
| POST | `/auth/register` | 自助註冊（需 `REGISTER_ENABLED=true`） |
| POST | `/auth/login` | 帳號密碼登入，回傳 JWT token（支援 `remember` 參數延長至 30 天）|
| GET | `/auth/me` | 取得當前登入使用者資訊 |
| POST | `/auth/change-password` | 修改密碼 |
| PUT | `/auth/profile` | 更新顯示名稱 / 信箱 |
...
| 理財系統分類 | 首次啟動自動插入 16 個預設分類（10 支出 + 6 收入）；新使用者註冊後也自動初始化 |
| CSV 編碼 | UTF-8 BOM，可直接以 Excel 開啟並顯示中文 |
| 股票持倉計算 | 買入股數 − 賣出股數 = 持有股數；平均成本 = 買入總成本 ÷ 買入總股數 |
| 預算警示 | 使用率 ≥ 80% 顯示黃色，> 100% 顯示紅色 |
| 自助註冊 | `.env` 設定 `REGISTER_ENABLED=true` 開放，預設關閉（admin 在後台新增帳號）|
| 記住我 30 天 | 登入時勾選後 JWT 有效期延長至 30 天；前端同步以 localStorage 記錄過期時間 |
| 帳號資料隔離 | 每位使用者的收支 / 股票 / 預算 / 分類資料完全隔離，互不可見 |
